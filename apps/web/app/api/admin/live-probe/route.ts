...
// TEMPORARY diagnostic route -- fires one minimal live chat.completions
// call per given model directly against entry-gateway, to see the raw
// upstream error for models that failed in a benchmark run (benchmark
// results only store a summarized "All compatible upstream routes
// failed" string, not the underlying per-attempt error). Gated by the
// same AUDIT_ROUTE_SECRET pattern as the other admin diagnostic routes.
// Delete once the current investigation is resolved.
