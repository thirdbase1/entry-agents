...
## 2026-09-17: compaction telemetry (95% threshold + events table + admin card)

- Owner changed the auto-compaction trigger from 80% to **0.95 of the model's context window** (`AUTO_COMPACT_THRESHOLD` in `packages/agent/context-management/auto-compact.ts`). Model defaults to the 128K window class; Anthropic-class models have per-model overrides in the context-windows table.
- Why 0.95 and not 1.0: the estimate (`estimateMessagesTokens` = JSON chars / 4) is rough, and system prompt + tool schemas add ~15K real tokens per request (`SYSTEM_AND_TOOLS_OVERHEAD_TOKENS`) that the estimator only approximates. 0.95 leaves the last 5% as estimator error margin so we compact before an upstream hard context-length rejection, without giving up usable window.
- Compaction never deletes messages or writes a summary — it only collapses old tool-call payloads to a `[Older tool output removed by auto-compaction...]` notice, protecting the last 12 messages (`PROTECTED_RECENT_MESSAGES`) plus every tool call the protected tail still references. The model "knows where it stopped" because the sequence, roles, and recent context stay intact.
- Telemetry: `packages/agent/context-management/compaction-telemetry.ts` uses an `AsyncLocalStorage` sink (`runWithCompactionSink` + `emitCompactionEvent`). Crucially, the sink is established **per `runAgentStep` call** in `apps/web/app/workflows/chat.ts`, NOT per turn: the Workflow SDK can suspend/resume between steps, and an ALS context set once at turn start would be lost across a suspension boundary.
- Sink writes are fire-and-forget with a swallowed failure path — a telemetry outage must never break a live model step.
- Persistence: `compaction_events` table (migration `0056_compaction_telemetry.sql`) + `apps/web/lib/db/compaction.ts` (`recordCompactionEvent`, `getCompactionOverview`, `getCompactionEligibleTurnCount`).
- `getCompactionEligibleTurnCount` uses typed drizzle helpers (`gte`) — do NOT rewrite it as a raw `sql\`\` template with Date params; Neon serverless serializes raw-template Dates via `toString()` and Postgres rejects them (same class of bug as the 2026-09-15 billing/me 500). The eligible-turns approximation: a turn's per-step average context is `(input+cached tokens) / (tool_call_count + 1)`; >= 240K means it was near the 256K window even if no compaction fired.
- Admin dashboard: `AdminCompactionSection` (`admin-compaction-section.tsx`) shows firings, chats affected, avg % reclaimed, near-window turns, and a recent-firings table. The "near-window turns" stat is what distinguishes "compaction broken" from "contexts never got big enough" — with zero events but nonzero eligible turns, compaction is suspect.
- Test gotcha (re-confirmed): `maybeCompactMessages` keeps the message COUNT constant (payloads collapse in place), so tests must assert on serialized size, not `result.length`. Also, `createInertPlaceholderModel("unknown-model-x")` falls back to the 128K default window — use it to build histories that predictably cross the 0.95 line (~122K tokens); real model IDs resolve to their per-model window (e.g. 200K for claude-4.5-sonnet) and need proportionally bigger fixtures.

## 2026-09-17: read-before-edit gate (hash-gated edits/writes)

- New `packages/agent/tools/read-state.ts`: every successful read (full, partial, tail, empty) records a hash of the FULL normalized file content; `edit` and `write`-overwrite now refuse files that were never read ("File has not been read yet") or changed since the last read ("changed since it was last read"). The model can only change what it has actually seen, in the version it last saw — bash sed / git checkout / subagent writes all trip the stale check.
- Hash basis is `normalizeFileContent` output (BOM-stripped, CRLF-normalized) in all three tools, so a CRLF file read by `read` and re-hashed by `edit` from raw bytes still matches — no false stale trips.
- State durability is DERIVATION, not storage: every read/edit/write success output carries `contentHash`, and `open-agent.ts` prepareStep replays the message history into the store (`rebuildReadFileState`, last write wins) on every model step. Survives refresh/resume/suspend-resume with no DB table to drift. Deliberate side effect: once compaction collapses an old read payload to a placeholder, the hash is no longer derivable, so the model must re-read — post-compaction it genuinely hasn't seen the file.
- The tools' own success outputs update the live Map (`ensureReadFileState`, lazily attached to experimental_context so it survives context spread in prepareStep), so consecutive edits never force re-reads. Write of a brand-NEW file needs no read; overwrite of an existing file does (a blind overwrite is worse than a blind edit).
- Subagents (executor/design) get a fresh `readFileState` per run — a subagent must read before editing, same rule; parent reads don't carry over.
- Tool result shape changes: `read` success outputs gained `contentHash` (full, unchanged-notice, and empty-file paths); `edit`/`write` successes gained `contentHash`; gate failures return `{ success:false, error, gate: "unread"|"stale" }`. UI renderers ignore the extra fields (web tsc clean).
- 20 new tests (read-state.test.ts rebuild/gate policy + read-gate.test.ts tool-level: unread/stale/chain/partial/CRLF/empty/overwrite/new-file/derivation).

## 2026-09-17: broken FK syntax in migrations 0056/0057 killed 3 deploys

- Symptom: every deploy since bfb15f5 (compaction telemetry) failed the build at `web:build` with `DrizzleQueryError: DO $$ BEGIN ... PostgresError: schema "users" does not exist`. Production stayed on cd37cae (Sept 15 pricing swap) — compaction telemetry, upstream adoption, and the CC read tool were NOT live despite "pushed" memories. Runtime-log checks are part of "pushed, auto-deploy": verify the deploy, don't just trust the push.
- Root cause: hand-written `REFERENCES "users"."id"` — Postgres parses a two-part quoted name as SCHEMA."TABLE", i.e. schema `users`, table `id`. Correct column FK is `REFERENCES "users" ("id")` (parenthesized column list). The migrations never applied; both CREATE TABLEs aborted before running, so no partial-state cleanup was needed.
- Fix: correct the two files in place (migrations are append-only in general, but an unapplied, always-failing migration file MUST be edited in place — otherwise the build stays wedged forever).
- LESSON: any new migration with an FK must use `REFERENCES "table" ("column")`, never `"table"."column"`. Search the diff for `"users"."` as a smoke test before pushing.

## 2026-09-17: Workflow SDK restricted-bundle regression — compaction sink in workflow scope

- Symptom: after the 0056/0057 FK fix cleared the migration error, builds STILL failed at the workflow compile: 21 `workflow-node-module-error` violations (`path` x10, `node:fs/promises`, `node:child_process`, `node:async_hooks`, `postgres`, `nanoid`, plus `Could not resolve "pkce-challenge"` via @ai-sdk/mcp). Same SDK version (workflow 5.0.0-beta.40) had compiled the cd37cae graph clean two days earlier — Vercel-side enforcement changed, but the strictness is the SDK's documented direction, so fix forward, don't chase the flip.
- Root cause: bfb15f5 put `runWithCompactionSink` (from `@open-agents/agent`) and `recordCompactionEvent` (from `@/lib/db/compaction`) as STATIC imports CALLED DIRECTLY in the `runAgentWorkflow` ("use workflow") body. The Workflow SDK's bundler pulls a statically-imported symbol's entire module graph into the restricted workflow bundle when referenced from workflow scope: async_hooks from compaction-telemetry, and via agent/index the WHOLE agent package (every tool's `path`/`fs`/`child_process` imports, skills, sandbox/local, mcp → pkce-challenge), plus postgres/nanoid from lib/db. Step-only static imports (e.g. `createMcpToolSet`, only referenced inside runAgentStep) were fine — the plugin only errors on builtins that SURVIVE tree-shaking in the emitted workflow chunk.
- Fix: moved the sink INSIDE `runAgentStep` ("use step"): dynamic-import `runWithCompactionSink`/`recordCompactionEvent` (same pattern as resolveChatModelSelection/isUserAdmin in that file), and wrap the `webAgent.stream()` + toUIMessageStream for-await consumption in the sink. Per-step scoping preserved (suspend/resume between steps can't lose the ALS context); every emitCompactionEvent (main model + subagents) still fires inside the sink.
- LESSON: the workflow function body may only call "use step" functions, Workflow SDK APIs, and pure helpers. Any static import from `@open-agents/agent`, `@/lib/db/*`, or anything touching Node built-ins must be a dynamic `await import()` INSIDE a "use step" function, never referenced from the workflow body. When a step needs to observe something happening inside it (like telemetry), wrap inside the step, not around the step call.
- Reproduced and verified locally: `pnpm exec next build` in apps/web went from 21 errors to `✓ Compiled workflows in 1.7s (129 steps, 5 workflows)` + full build EXIT=0.

- **2026-09-17, metrics semantics must be explicit across Gateway and Entry:** Gateway `gw_metrics_*`.`tokens_input` is uncached-only after route normalization, with `tokens_cache_read`/`tokens_cache_write` separate. Entry `usage_events.input_tokens` is the full prompt total and `cached_input_tokens` is a detail. Gateway's headline incorrectly used uncached input + output, hiding most prompt traffic; fixed it to include cache reads/writes. Entry's usage UI now exposes cache-read tokens instead of hiding them. Never compare or reuse the two dashboards' token formulas without checking the raw field semantics.

## 2026-09-22: sandbox drives — one per session, `get()` cannot add mounts, and the restore story

- **A Drive attaches to exactly ONE sandbox at a time.** The SDK's `DriveMetadata` has singular `currentSessionId` / `currentSandboxName`. Sandboxes are named `session_<sessionId>` (one per user session), so a single shared drive serialises concurrent users behind each other and lets one workspace exhaust another's space. Per-session drive name: `entry-agents-session-<sessionId>`, 8 GiB.
- **`Sandbox.get()` does NOT accept `mounts`.** Only `create()`, `getOrCreate()`, and `update()` do. `GetSandboxParams` is `{ name, resume?, signal?, onResume? }`. Consequence for our config: a sandbox RESUMED via `connect()` keeps whatever mounts it was CREATED with — there is no way to attach a drive to an existing sandbox at connect time. `sandbox.update({ mounts })` does exist, but it "replaces all current mounts and applies to the NEXT session", so it is not a connect-time fix either.
- **Therefore the drive restore story is: create-time only, and migration is the path that actually carries data.** In our configuration (`persistent: false`, owner-confirmed 2026-08-28) a stopped sandbox's filesystem is discarded, so resume never restores anything anyway. The drive is attached when the sandbox is CREATED, and the workspace survives a sandbox's death only because `migration.ts` mounts the SAME per-session drive on the fresh sandbox and then `restoreWorkspacePayload()` writes into it. Net: drive = durable backing store; migration = the mechanism that repopulates a new VM from it. A session that dies WITHOUT migrating loses whatever was not yet on the drive.
- **Retention must be keyed on the DRIVE's `updatedAt`, not the session's.** A drive stops being touched the moment its sandbox detaches, so drive age is the honest "nothing is writing here" signal. 3 days of no drive writes; attached drives (`currentSandboxName` set) are never candidates.
- **`@vercel/sandbox` is only declared by `packages/sandbox`.** Importing it from `apps/web` compiles fine locally under `tsc` but fails the Turbopack build with `Module not found: Can't resolve '@vercel/sandbox'`. Keep SDK usage inside `packages/sandbox` and re-export.
- **Vercel's runtime-log endpoints are unreachable from this environment.** `/v1/projects/{id}/deployments/{dpl}/runtime-logs` times out (no response) and the MCP `get_runtime_logs`/`get_runtime_errors` tools return `403 You don't have permission to access this resource` even with an OWNER token on the team. Build events (`/v3/deployments/{id}/events`) work fine. Do not burn turns re-probing; use the dashboard or `vercel logs`.
- **`lessons-learned.md` is the rulebook and must be read BEFORE changing sandbox/persistence behaviour.** The 2026-08-28 entries document an explicit owner decision (non-persistent default, real fix was the early-hibernate bug in `getLifecycleDueAtMs`) that an agent can silently override by "fixing" persistence the obvious way. Read the relevant dated entries first.

## 2026-09-22: sub-cent billing (whole-cent ledger silently billed cheap steps at zero)

- **Root cause: `Math.round(costUsd * 100)` before `debitUsage`, and `debitUsage` early-returns on `costCents <= 0`.** A step costing $0.004–$0.0001 rounds to 0 cents and is then DISCARDED — not rounded down, dropped entirely. Measured: ten $0.004 steps = 4 cents of real cost billed as 0. On a cheap-model catalog that is systematic free usage, and it is invisible because nothing in the ledger records the shortfall.
- **Not a per-step-granularity problem.** The owner reasoned "per step will make us lose money, charge per token" — the granularity was NOT the cause. Token-level charging is not achievable through the AI SDK anyway (stream parts do not report a running cost; `finish-step` is the earliest point a step's cost exists). The loss was purely the rounding + the `<= 0` early return.
- **Fix: `lib/billing/usage-accrual.ts` — a per-turn carry accumulator.** `settleStepCost(state, costUsd)` folds each step at full precision and returns only the whole cents to debit; the remainder stays pending in `[0, 1)`. DB schema untouched (`integer` columns stay integer). Makes billing slightly LAZY by design — a step settles only once the carry crosses a cent — which is the same tradeoff the integer column forces and is strictly fairer than undercharging.
- **Three sites, one shape.** `chat.ts:2865` (real-time main-model debit), `chat-post-finish.ts:512` (subagent debit), plus the guard in `realtime-spend-cap.ts:382`. Apply the same accrual to all of them; a cheap subagent was previously billing nothing at all.
- **Spend enforcement must see EVERY step's real cost, not just what settled.** Balance/window counters decrement by the settled cents, but the cap comparison must include the pending carry, or a user can run unlimited sub-cent steps and never trip the zero-balance guard.

## 2026-09-22: `0057_github_repo_index` — a committed migration that never ran

- **A `.sql` file is NOT a migration until it is registered in `meta/_journal.json`.** Drizzle's migrator reads the journal, not the directory. `0057_github_repo_index.sql` was committed (7e857f6) with its `schema.ts` table and all the code that reads/writes it, but the journal entry was never added — so `github_repo_index` never existed in production.
- **Symptom, not cause: the GitHub API was fine.** The repo selector 500'd with `relation "github_repo_index" does not exist` (42P01) on the INSERT inside `saveUserRepoIndex`, but `listUserInstallationRepositories` had already returned the correct repo list. The cache WRITE was the only thing that broke, and it took the whole flow down. Do not read a 500 here as a GitHub/auth problem.
- **This repo's hand-written migrations have NO snapshots, and that is correct.** 30+ journal entries have no `<idx>_snapshot.json`. Snapshots are for `drizzle-kit generate`, never for `migrate`. Do not "fix" missing snapshots; the journal is the only ledger that matters. Check with a journal-integrity test, not by eyeballing the folder.
- **Fix (two parts, both needed).** (1) Register the migration: added journal `idx: 58`, `tag: "0057_github_repo_index"`. Note the duplicate `0057_` prefix — that is allowed here because the tag, not the numeric prefix, is the key. (2) Make the cache best-effort: `getCachedUserRepoIndex` and `saveUserRepoIndex` are now `.catch()`-wrapped so a cache failure can never fail the request. The repo index is a pure optimisation (10-min TTL over `fetchModelCostCatalog`-style GitHub paging); the live GitHub response is already correct without it. Mirror the existing `getLastRepoByUserId().catch(() => null)` pattern that was already on the next line.
- **Regression test: `lib/db/migrations-journal.test.ts`.** Asserts every `.sql` has a journal entry (the valuable one), every entry has a file, indexes are contiguous from 0, and tags are unique. Verified it FAILS against the pre-fix journal (2 failures) and PASSES after — a real guard, not a rubber stamp.
- **LESSON: after adding any migration, assert journal coverage in CI.** `pnpm db:check` only catches schema-vs-migration DRIFT; it does not notice an orphaned file that drizzle simply never sees. That gap is what let this ship and stay broken in production across deploys.
