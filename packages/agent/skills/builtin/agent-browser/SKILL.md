...
---
## First-time setup (run once per sandbox session)

`agent-browser` is not preinstalled in this sandbox. Before your first command in a session, check and install it:
...
This downloads a Chrome for Testing binary the first time (roughly 1-2 minutes). Skip this check on later
commands within the same session — the binary persists for the life of the sandbox.

If `--with-deps` reports it could not install every required library, retry with plain `agent-browser install`
and proceed anyway; most sites still render fine without every optional system library.
